...
SUBJECTIVE_WEIGHTS = {
    "fatigue": 0.20, "doms": 0.20, "sleep": 0.15,
    "stress": 0.20, "desire": 0.10, "freshness": 0.15
}
...
def Dz_eff(rk: np.ndarray, W: np.ndarray, M_obj: float, M_subj: float,
           cpain: np.ndarray = None, alpha0: float = ALPHA0) -> np.ndarray:
...
# --- Мулти-денна сума на "дефицити" и готовност ---
def ready_curve_multi(t_days, R0_list, D_list, t0_list):
    """
    Сумира ефектите от няколко последователни тренировки.
    t_days: np.array (T)
    R0_list: [np.array(n_zones), ...]   # по една за всеки ден
    D_list:  [np.array(n_zones), ...]
    t0_list: [float, ...]  # момент на всяка сесия в дни (0, 1, 2, ...)
    Връща: Ready%(zones x T)
    Модел: дефицитът от всяка сесия е (1-R0)*exp(-ln20*(t-t0)/D), t>=t0.
           Дефицитите се сумират и се клипват в [0,1].
    """
    T = len(t_days)
    n_zones = R0_list[0].shape[0]
    total_deficit = np.zeros((n_zones, T), dtype=float)

    for R0, D, t0 in zip(R0_list, D_list, t0_list):
        dt = t_days - float(t0)
        mask = dt >= 0.0
        decay = np.zeros((n_zones, T), dtype=float)
        if np.any(mask):
            decay[:, mask] = np.exp(-LN20 * (dt[mask][None, :] / (D[:, None] + 1e-9)))
        deficit = (1.0 - R0)[:, None] * decay
        total_deficit += deficit

    total_deficit = np.clip(total_deficit, 0.0, 1.0)
    ready = 100.0 * (1.0 - total_deficit)
    return ready
